...
###############################################
# Usage: perl parse_phmmer.py -i [phmmerfile] -s [self_phmmer] -o [outfile] [options]
#
# Jen Wisecaver
# 20201018
################################################
...
def usage():
    print('\n  Usage: '+sys.argv[0]+' -i <phmmerfile> -s <self_phmmer file> -o <outfile> [options]')
    print("    -i|--infile <FILENAME>  path to phmmer file")
    print("    -s|--selfsearch <FILENAME>  path to self phmmer file")
...
for line in fi:
    ...
    if qseqid in queryDict:
        ...
        if normbit > 1 and normbit < 1.1:
            ...
            sys.stdout.write('\tWARNING: normalized bitscore(' + str(normbit) + ') greater than 1 :' + line + '\n')
            normbit = 1
    else:
